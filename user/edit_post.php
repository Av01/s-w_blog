...
if(isset($_POST['submit'])){
   extract($_POST);
   try{
      $stmt = $db -> prepare('UPDATE blog_posts SET postTitle = :postTitle, postDesc = :postDesc, postCont = :postCont, postDate = :postDate WHERE postID = :postID');
      $stmt -> execute(array(
         ':postID' => $_GET['postID'],
         ':postTitle' => $postTitle,
         ':postDesc' => $postDesc,
         ...
         header('Location: index.php?action=Post edited');
         exit;  
   }
             ...
             try{
	   		...
	   		echo'<article class="entry">';
                  ...
                  echo'<form action = "" method = "POST">';
                     echo"<input type='hidden' size = '35' name='postID' value='".$row['postID']."'>";
							echo'<label>PostTitle:</label>';
							echo'<input type="text" size ="35" name = "postTitle" value = "'.$row['postTitle'].'">';
                     echo'<label>Description:</label>';
                     echo'<input type="text" size = "35" name = "postDesc" value = "'.$row['postDesc'].'">';
                     echo'<label>Content:</label>';
                     echo'<textarea rows="10" cols="50" name = "postCont">"'.$row['postCont'].'"</textarea>';
                     echo '<input type = "submit" name = "submit" value = "Edit Post">';
                  echo '</form>';
						echo'</div>';
				echo'</article>';
         } catch(PDOException $e)
