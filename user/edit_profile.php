...
if(isset($_POST['submit'])){
   extract($_POST);
   try{
      $stmt = $db -> prepare('UPDATE mem_details SET mem_name = :mem_name, mem_email = :mem_email, mem_age = :mem_age, mem_gen =
         :mem_gen, mem_des = :mem_des WHERE mem_uname = :username');
      $stmt -> execute(array(
         ':mem_uname' => $_SESSION['username'],
         ':mem_name' => $name,
         ':mem_email' => $email,
         ':mem_age' => $age,
         ':mem_gen' => $gender,
   ...
   <div id="content-wrap">
   	...
   	<div class="row">
   		...
   		<div id="main" class="eight columns">
   		    <?php
                try {
               		$stmt = $db->prepare('SELECT mem_name, mem_email, mem_age, mem_gen, mem_des FROM mem_details WHERE mem_uname = :username');
            ...
            while($row = $stmt->fetch()){
	   		...
	   		echo'<article class="entry">';
                  ...
                  echo'<form action = "" method = "POST">';
							...
							echo'<input type="text" name = "name" size="35" value = '.$row['mem_name'].'>';
                     echo'<label>Email:</label>';
                     echo'<input type="text" name = "email" size="35" value = '.$row['mem_email'].'>';
                     echo'<label>Age:</label>';
                     echo'<input type="number" name = "age" size="35" value = '.$row['mem_age'].'>';
                     echo'<label>Gender:</label>';
                     echo'<input type="text" name = "gender" size="35" value = '.$row['mem_gen'].'>';
                     echo'<label>About Me:</label>';
                     echo'<textarea name = "desc" rows="10" cols="50">'.$row["mem_des"].'</textarea>';
